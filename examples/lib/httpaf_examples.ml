  ...
  let print ~on_eof response response_body =
    ...
    | { Response.status = `OK; _ } as response ->
      ...
      let rec on_read bs ~off ~len =
        ...
        Body.Read.schedule response_body ~on_read ~on_eof
      in
      Body.Read.schedule response_body ~on_read ~on_eof;
    | response ->
...
module Server = struct
  let echo_post reqd =
    ...
    | { Request.meth = `POST; headers; _ } ->
      ...
      let rec on_read buffer ~off ~len =
        Body.Write.bigstring response_body buffer ~off ~len;
        Body.Read.schedule request_body ~on_eof ~on_read;
      and on_eof _ =
        Body.Write.close response_body
      in
      Body.Read.schedule (Reqd.request_body reqd) ~on_eof ~on_read
    | _ ->
  ...
  let error_handler ?request:_ error start_response =
    ...
    | `Exn exn ->
      Body.Write.string response_body (Exn.to_string exn);
      Body.Write.string response_body "\n";
    | #Status.standard as error ->
      Body.Write.string response_body (Status.default_reason_phrase error)
    end;
    Body.Write.close response_body
  ;;
end
